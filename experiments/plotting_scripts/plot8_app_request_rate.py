...
def plot_app_request_rate(sim_dir, scenario_key, output_dir, max_steps=None, top_n=5):
    ...
    # Panel 1: Stacked area for total system request load (all applications)
    ...
    ax1.stackplot(steps, req_matrix_all, colors=colors_all, alpha=0.7)
    ax1.set_ylabel(y_label)
    ax1.set_title(f'Stacked System Load. All the Applications ({n_apps} apps)',
                  fontsize=11, loc='left', pad=4)
                  
    # Plot markers for affected apps
    from matplotlib.lines import Line2D
    cum_req_all = np.cumsum(req_matrix_all, axis=0)
    has_viral_start = False
    has_viral_end = False
    for (step, app_name, evt_type) in affected_events:
        if app_name in sorted_apps and step < num_steps and app_name not in less_popular_apps:
            ...
            y_pos = cum_req_all[app_idx, step]
            marker = '^' if evt_type == 'viral_cascade_start' else 'v'
            if evt_type == 'viral_cascade_start': has_viral_start = True
            if evt_type == 'viral_cascade_end': has_viral_end = True
            
            ax1.scatter([step], [y_pos], marker=marker, s=80, 
                        facecolor=app_color_map[app_name], edgecolor='black', zorder=10)
                        
    # Add legend for events if they occurred
    legend_elements = []
    if has_viral_start:
        legend_elements.append(Line2D([0], [0], marker='^', color='w', markerfacecolor='grey', markeredgecolor='black', markersize=8, label='Start Viral Cascade'))
    if has_viral_end:
        legend_elements.append(Line2D([0], [0], marker='v', color='w', markerfacecolor='grey', markeredgecolor='black', markersize=8, label='End Viral Cascade'))
    if legend_elements:
        ax1.legend(handles=legend_elements, loc='upper left', fontsize=9, title="Events")

    # Panel 2: Stacked area for less popular applications (non-viral)
    ...
    ax2.stackplot(steps, req_matrix_less, colors=colors_less, alpha=0.7)
    ax2.set_ylabel(y_label)
    ax2.set_xlabel('Simulation Step')
    ax2.set_title(f'Stacked System Load. Less Popular Applications ({n_less} apps)',
                  fontsize=11, loc='left', pad=4)

    cum_req_less = np.cumsum(req_matrix_less, axis=0)
    for (step, app_name, evt_type) in affected_events:
        if app_name in less_popular_apps and step < num_steps:
            app_idx = less_popular_apps.index(app_name)
            y_pos = cum_req_less[app_idx, step]
            marker = '^' if evt_type == 'viral_cascade_start' else 'v'
            ax2.scatter([step], [y_pos], marker=marker, s=80, 
                        facecolor=app_color_map[app_name], edgecolor='black', zorder=10)

    plt.tight_layout(rect=[0, 0, 1, 0.95])

    ensure_output_dir(output_dir)
    filepath = os.path.join(output_dir, f'plot8_app_request_rate_{scenario_key}.pdf')
    fig.savefig(filepath, format='pdf')
    plt.close(fig)
    print(f'  [Plot 8] Saved 2-panel figure: {filepath}')
    ...
    ax_clip.stackplot(steps, req_matrix_all, colors=colors_all, alpha=0.7)
    ax_clip.set_ylabel(y_label)
    ax_clip.set_xlabel('Simulation Step')
    ax_clip.set_title(f'Stacked System Load. All the Applications ({n_apps} apps)',
                      fontsize=11, loc='left', pad=4)
    ...
    for (step, app_name, evt_type) in affected_events:
        if app_name in sorted_apps and step < num_steps:
            ...
            y_pos = cum_req_all[app_idx, step]
            marker = '^' if evt_type == 'viral_cascade_start' else 'v'
            ax_clip.scatter([step], [y_pos], marker=marker, s=80, 
                            facecolor=app_color_map[app_name], edgecolor='black', zorder=10)
    if legend_elements:
        ax_clip.legend(handles=legend_elements, loc='upper left', fontsize=9, title="Events")

    plt.tight_layout(rect=[0, 0, 1, 0.93])
    filepath_total = os.path.join(output_dir, f'plot8_app_request_rate_{scenario_key}_total.pdf')
    fig_clip.savefig(filepath_total, format='pdf')
    plt.close(fig_clip)
    print(f'  [Plot 8] Saved total clip figure: {filepath_total}')
